...
export function validateThreshold(eventType, threshold) {
  ...
  switch (eventType) {
    ...
    case 'earthquake':
      return num(threshold.minMagnitude) ? null : 'earthquake threshold needs a numeric "minMagnitude"';
    case 'wildfire':
      return threshold.withinMiles == null || num(threshold.withinMiles)
        ? null
        : 'wildfire "withinMiles" must be a number when present';
    case 'weather_alert':
      return SEVERITY_VOCAB.includes(String(threshold.severityAtLeast || '').toLowerCase())
        ? null
    ...
    case 'water_gauge':
      ...
      return num(threshold.gageHeightGt) ? null : 'water_gauge threshold needs a numeric "gageHeightGt"';
    case 'flood':
      return null;
    default:
...
async function findOrCreateLocation({ address, lat, lon }) {
  let loc = { lat, lon, stateFips: null, countyFips: null, tract: null, address };
  if (address) {
  ...
  const { rows } = await query(
    `INSERT INTO locations (address, lat, lon, geom, state_fips, county_fips, census_tract)
     ...
     RETURNING id`,
    [loc.address || null, loc.lat, loc.lon, loc.stateFips ?? null, loc.countyFips ?? null, loc.tract ?? null],
  );
...
subscriptionsRouter.post('/', async (req, res) => {
  const { address, lat, lon, event_type, threshold, delivery_method, delivery_target } = req.body || {};
  ...
  if (!delivery_target) {
    return res.status(400).json({ error: 'delivery_target (Discord webhook URL) is required' });
  }
  if (!address && !(Number.isFinite(Number(lat)) && Number.isFinite(Number(lon)))) {
    return res.status(400).json({ error: 'provide an address or numeric lat/lon' });
  }
  ...
  try {
    ...
    if (!locationId) return res.status(404).json({ error: 'location could not be resolved' });
    ...
    const { rows } = await query(
      ...
      [locationId, event_type, JSON.stringify(threshold), delivery_method, delivery_target],
    );
...
// GET /api/subscriptions — list (dev convenience; delivery_target omitted).
...
// PATCH /api/subscriptions/:id — pause/resume.
subscriptionsRouter.patch('/:id', async (req, res) => {
  const { active } = req.body || {};
  if (typeof active !== 'boolean') {
    return res.status(400).json({ error: 'body must include a boolean "active"' });
  }
  try {
    const { rows } = await query(
...
subscriptionsRouter.delete('/:id', async (req, res) => {
  try {
    const { rowCount } = await query('DELETE FROM alert_subscriptions WHERE id = $1', [req.params.id]);
